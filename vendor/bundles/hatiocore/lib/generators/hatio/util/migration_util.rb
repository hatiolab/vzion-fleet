  ...
  module Generators
    class MigrationUtil
      ...
      def self.create_table_migration(className, tableName, attributes, options, pkColumnName)
        ...
        output << "\tdef change\n"
        output << "\t\tcreate_table #{tableName.to_sym.inspect} do |t|\n"
        output << "#{Hatio::Generators::MigrationUtil.add_migration_columns(attributes, pkColumnName)}"
        output << "\t\tend\n\n"

        output << "#{MigrationUtil.add_migration_index(tableName, attributes)}"
