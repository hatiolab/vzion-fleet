    ...
    class ResourceViewUtil
      ...
      def self.getFormColumn(domain, col)
        ...
        if(col.name.end_with?('_yn'))
          "{ name : '#{col.name}', fieldLabel : T('label.#{col.name}'), xtype : 'yesnopicker' }"
          ...
          "{ xtype : 'datefield', name : '#{col.name}', fieldLabel : T('label.#{col.name}'), format : T('format.datetime') }"
        elsif(checkNumberColumn(column_type))
      ...
      def self.getSearchFormColumnByRef(domain, col)
        if(col.ref_type == 'CommonCode')
          return "{ fieldLabel : T('label.#{col.name}'), name : '#{col.name}-eq', xtype : 'codesearchcombo', commonCode : '#{col.ref_name}', valueField : 'name', displayField : 'name' }"
        elsif(col.ref_type == 'Entity')
      ...
      def self.getSearchFormColumn(domain, col)
        column_type = col.column_type
        if(col.name.end_with?('_yn'))
          return "{ fieldLabel : T('label.#{col.name}'), name : '#{col.name}-eq', xtype : 'yesnopicker' }"
        elsif(checkTextColumn(column_type))
          ...
          output = "{ fieldLabel : T('label.#{col.name}'), name : '#{col.name}-eq', xtype : 'datefield', format : T('format.date') }"
          return output
        elsif(column_type == 'datetime' || column_type == 'timestamp')
