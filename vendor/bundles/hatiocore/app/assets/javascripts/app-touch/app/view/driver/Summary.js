Ext.define('FleetTouch.view.driver.Summary', {
	...
	constructor : function(config) {
        config.items = [this.buildDriverInfo(), {
			...
			data: null,
            tpl: [
				'<div class="iconTime">' + T('label.total_run_time') + '<span>{total_runtime} ' + T('label.parentheses_min') + '</span></div>',
		...
		this.on('painted', function() {
			FleetTouch.setting.on('driver', this.refresh, this);
			
			this.refresh();
		});
		
		this.on('erased', function() {
			FleetTouch.setting.un('driver', this.refresh, this);
		});
	...
	refresh : function() {
		...
		if(FleetTouch.setting.get('driver') === this.driver) 
			return;
			
		var store = Ext.getStore('DriverSummaryStore');
		this.driver = FleetTouch.setting.get('driver');
		store.setParams({id : this.driver});
		...
		store.load(function(records) {
			...
			data.effcc_of_month = data.effcc_of_month.toFixed(2);
			data.total_distance_mile = (data.total_dist * 0.621371192237334).toFixed(2);
			...
			if(imageClip) {
				...
				dimage.setSrc('assets/app-touch/resources/images/bgDriver.png');
			}
		});
	},
	...
	buildDriverInfo: function() {
        return {
            ...
            },
			{
                ...
                data: null,
                tpl: [
                '<div class="infoID">'  + T('label.name') + ' : {id}</div>',
            ...
            },
			{
                ...
                data: null,
                tpl: [
                '<div class="infoID">' + T('label.id') + ' : {name}</div>',
                '<div class="infoText">' + T('label.social_id') + ' : {social_id}</div>',
                '<div class="infoText">' + T('label.phone_x', {x : 1}) + ' : {phone_no}</div>',
                ]
			}]
        }
    },
	...
	buildRunningInfo : function() {
		return {
			...
			tpl : [
			'<div class="distance">',
				'<div class="total">'+ T('label.total_distance') +'<span class="km">{total_dist} km</span><span class="mile">{total_distance_mile} mile</span></div>',
