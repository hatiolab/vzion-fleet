Ext.define('FleetTouch.view.monitor.Incident', {
	extend : 'Ext.TabPanel',
	
	requires: [
		'FleetTouch.view.chart.vehicle.Accel'
	],
	...
	setIncident : function(incident) {
		...
		FleetTouch.setting.set('driver', incident.get('driver_id'));
		FleetTouch.setting.set('vehicle', incident.get('vehicle_id'));
		...
		if (vehicleImageClip) {
			...
			vimage.setSrc('assets/app-touch/resources/images/bgVehicle.png');
		}
		incident.set('name', vehicleRecord.get('name'));
		incident.set('description', vehicleRecord.get('description'));
		...
		incident.set('driver_desc', driverRecord.get('description'));
		...
		this.sub('video').updateUrl([video_clip]);
	},
	...
	refreshTrack : function(store, records) {
		...
		if (first) {
			var start = new google.maps.Marker({
				...
				icon : 'assets/app-touch/resources/images/iconStartPoint.png',
				map : this.getMap()
			});
		...
		this.setMarker(new google.maps.Marker({
			position : location,
			icon : new google.maps.MarkerImage('assets/app-touch/resources/images/iconIncidentPoint.png',
