Ext.define('FleetTouch.view.monitor.InfoBack', {
    extend: 'Ext.Panel',
    ...
    setVehicle: function(vehicle) {
        ...
        if (!vehicle) {
			var vid = FleetTouch.setting.get('vehicle');
			if(vid) {
				...
				var did = FleetTouch.setting.get('driver');
				if(did) {
		...
		try {
			...
			if(vid && (vid !== FleetTouch.setting.get('vehicle')))
				FleetTouch.setting.set('vehicle', vid);
			if(did && (did !== FleetTouch.setting.get('driver_id')))
				FleetTouch.setting.set('driver', did);
		} finally {
		...
		if (vehicleImageClip) {
			if(FleetTouch.setting.get('app_mode'))
		...
		FleetTouch.map.getLocation(vehicle.get('lat'), vehicle.get('lng'), function(location) {
    ...
    buildVehicleInfo: function() {
        return {
            ...
            },
			{
                ...
                data: null,
                tpl: [
                '<div class="infoID {status}">{id} ({name})</div>',
				'<tpl if="driver_id">',
