Ext.define('FleetTouch.view.monitor.Track', {
	...
	addTrackLine : function(map, traces, line, avg_v, distance) {
		...
		path.forEach(function(point) {
			var marker = new google.maps.Marker({
				position : point,
				icon : 'assets/app-touch/resources/images/iconPin.png',
				map : map,
				visible : false,
				trace : traces[i++]
			});
		...
		function selectPath(e) {
			...
			FleetTouch.map.getLocation(lat, lng, function(address) {
				var content = [
		...
		function selectTrip(e) {
			...
			var content = [
				'<div class="bubbleWrap">',
					'<div class="close"></div>',
					'<div class="trackBubble">',
						'<div><span>'+ T('label.vehicle') +'</span>' + vehicleId + ' - ' + vehicle.get('name') + '</div>',
