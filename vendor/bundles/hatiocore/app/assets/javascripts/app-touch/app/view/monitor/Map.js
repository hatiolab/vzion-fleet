Ext.define('FleetTouch.view.monitor.Map', {
	...
	onSelectVehicle : function() {
		...
		store.filter('id', FleetTouch.setting.get('vehicle'));
	},
	
	onSelectDriver : function() {
		var store = Ext.getStore('VehicleFilteredStore');
		store.clearFilter(true);
		store.filter('driver_id', FleetTouch.setting.get('driver'));
	},
	...
	refreshMap : function(store) {
		...
		var images = {
			'Running' : 'assets/app-touch/resources/images/statusDriving.png',
			'Idle' : 'assets/app-touch/resources/images/statusStop.png',
			'Incident' : 'assets/app-touch/resources/images/statusIncident.png',
			'Maint' : 'assets/app-touch/resources/images/statusMaint.png'
		};
		...
		var markerHandler = function() {
			...
			var dimgsrc = 'assets/app-touch/resources/images/bgDriver.png';
			
			if(dr && dr.get('image_clip')) {
				dimgsrc = dr.get('image_clip');
			}
			
			var vimgsrc = 'assets/app-touch/resources/images/bgVehicle.png';
			...
			var content = [
				'<div class="bubbleWrap status'+ vr.get('status') +'">',
					...
					'<div class="infoCell">',
						'<img src="' + vimgsrc + '">',
						'<div class="showVehicleInfo">ID : ' + vr.get('description') + '<br/>'+ vr.get('name') + '</div>',
					'</div>',
					'<div class="infoCell">',
						'<img src="' + dimgsrc + '">',
							dr ? '<div class="showDriverInfo">ID : ' + dr.get('name') + '<br/>'+ dr.get('description') + '</div>'
			...
			if(!self.infowindow) {
				self.infowindow = FleetTouch.label.create({
					map : this.getMap(),
					xoffset : -110,
					yoffset : -150
				});
		...
		if(!bounds) {
			...
			setTimeout(function() {
				...
				self.getMap().setZoom(FleetTouch.setting.get('auto_max_zoom') || 16);
		...
		} else if(FleetTouch.setting.get('autofit')){ 
