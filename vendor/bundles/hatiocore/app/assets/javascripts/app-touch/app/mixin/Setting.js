Ext.define('FleetTouch.mixin.Setting', function() {
	...
	Ext.define('FleetTouch.mixin.Setting.Model', {
	    ...
	    config: {
	        ...
	        proxy: {
	            type: 'localstorage',
	            id  : 'fleettouch-settings'
	        }
	    }
	});

	var store = Ext.create('Ext.data.Store', {
		model : 'FleetTouch.mixin.Setting.Model',
		autoSync : true
	});
	...
	function setLocalSetting(name, value) {
		...
		var old;
		if(!record) {
			var set = Ext.create('FleetTouch.mixin.Setting.Model', {
				id : name,
				value : undefined
			});
			store.add(set);
	...
	Ext.define('FleetTouch.mixin.Setting.Inner', {
	...
	return {
		setting : Ext.create('FleetTouch.mixin.Setting.Inner')
	}
}());