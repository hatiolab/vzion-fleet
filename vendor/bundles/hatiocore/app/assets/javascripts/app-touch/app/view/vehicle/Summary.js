Ext.define('FleetTouch.view.vehicle.Summary', {
	
	extend : 'Ext.Panel',
		
	xtype : 'vehicle_summary',
	
	requires : [ 'FleetTouch.store.VehicleSummaryStore' ],
	...
	refresh : function() {
		...
		store.load(function(records) {
			...
			vehicleInfo.effcc_of_month = vehicleInfo.effcc_of_month.toFixed(2);
			...
			self.down('[itemId=consumableInfo]').setData(Ext.Array.map(sorted.slice(0,3), function(record) {
				record.health_rate_max = Math.min(1, record.health_rate).toFixed(2);
				record.health_rate = (record.health_rate).toFixed(2);
				return record;
			}));
	...
	buildVehicleInfo: function() {
        return {
            ...
            },
			{
                ...
                data: null,
                tpl: [
                '<div class="infoID">' + T('label.reg_no') + ' : {name}</div>',
