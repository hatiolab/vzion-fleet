...
class EntitiesController < ResourceMultiUpdateController
    ...
    def generate_views
      ...
      viewType = 'list-item' if(detailViewType != 'none' && viewType == 'list')
      runtimeOption = params[:runtime_option] || ''
      skipStore = (params[:skip_store] == 'true' ? 'y' : 'n') || 'n'
      parentMenu = params[:parent_menu] || ''
      commandStr = "rails generate hatio:resource_view #{params[:resource_name]} #{runtimeOption} --domain_id=#{Domain.current_domain.id} --bundle=#{params[:bundle]} --view_type=#{viewType} --detail_view_type=#{detailViewType} --skip_store=#{skipStore} --parent_menu=#{parentMenu}"
      puts commandStr
    ...
    def generate_api
      ...
      debug_print "#{params[:domain_id]}"
      # rails generate hatio:resource_api Customer --force --id_type=auto-increment --bundle=common --history=n
      # rails generate hatio:resource_api Supplier --id_type=auto-increment --bundle=Common --history=n --domain_name=System
      commandStr = "rails generate hatio:resource_api #{params[:resource_name]} #{runtimeOption} --domain_name=#{Domain.current_domain.name} --id_type=#{idType} --bundle=#{bundle} --history=#{historyOption}"
  ...
  private
    ...
    def resource_params
      [ params.require(:entity).permit(:name, :description, :bundle, :list_infographic_id, :item_infographic_id) ]
    end
end